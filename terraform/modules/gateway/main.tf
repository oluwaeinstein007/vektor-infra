...
# vektor-backend and vektor-web; Keycloak is the OIDC provider behind it (§10.2).
#
...
resource "helm_release" "kong" {
  ...
  # live next to each service's manifest in vektor-backend's (or
  # vektor-web's) own repo, rather than a stateful Kong Postgres this repo
  # would have to own too.
  set {
    name  = "env.database"
    value = "off"
  }
